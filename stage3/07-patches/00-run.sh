...
echo -e "\e[32m### Installing patched files ###\e[0m" >&2
...
echo -e "\e[32m### Installing /usr/bin/ files ###\e[0m" >&2
...
# /etc/update-motd.d/
# must be executable (755)
echo -e "\e[32m### Installing /etc/update-motd.d/ files ###\e[0m" >&2
install -v -m 755 files/01-motd "${ROOTFS_DIR}/etc/update-motd.d/01-motd"

# /etc/NetworkManager/
echo -e "e[32m### Installing NetworkManager configurations ###\e[0m" >&2
...
echo -e "\e[32m### Removing unnecessary files ###\e[0m" >&2
...
echo -e "\e[32m### Setting NM Unmanaged udev rules to 0 ###\e[0m" >&2
